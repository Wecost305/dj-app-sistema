// --- SERVIDOR SAAS MULTI-TENANT PARA DJ APP (VERSIÓN FINAL CON REGISTRO CORREGIDO) ---
...
// VERSIÓN NUEVA Y UNIVERSAL (Funciona en Local y en Render)

// 5.1. Define la configuración para el entorno local
const localPoolConfig = {
    user: 'postgres',
    host: 'localhost',
    database: 'dj_app_db',
    password: 'Samva.Wm5', // Tu contraseña local
    port: 5432,
};

// 5.2. Define la configuración para producción (Render) usando la DATABASE_URL
const prodPoolConfig = {
    connectionString: process.env.DATABASE_URL, // Render inyecta esta variable
    ssl: {
        rejectUnauthorized: false // Necesario para las conexiones seguras de Render
    }
};

// 5.3. Elige la configuración correcta basándose en el entorno
//    process.env.NODE_ENV se establece a 'production' automáticamente en Render
const pool = new Pool(process.env.NODE_ENV === 'production' ? prodPoolConfig : localPoolConfig);
...
passport.use(new LocalStrategy({ usernameField: 'email' }, async (email, password, done) => {
    try {
        ...
        if (result.rows.length === 0) return done(null, false, { message: 'Email no registrado.' });
...
passport.deserializeUser(async (id, done) => {
    try {
        const result = await pool.query('SELECT id, email, dj_name, public_slug, is_accepting_requests, role FROM djs WHERE id = $1', [id]);
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        // ¡CORRECCIÓN! Generar un slug único y seguro
        const baseSlug = dj_name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
        ...
        await pool.query(
            ...
            [email, hashedPassword, dj_name, uniqueSlug]
        );
        // ¡CORRECCIÓN! Enviar una respuesta JSON válida
        res.status(201).json({ message: 'Registro exitoso' });
    } catch (err) {
        console.error('ERROR EN /api/register:', err);
        // ¡CORRECCIÓN! Enviar un error JSON válido
        if (err.code === '23505') { // Código de error de PostgreSQL para violación de unicidad
            return res.status(409).json({ message: 'El email o el nombre de DJ ya están en uso.' });
        }
...
// VERSIÓN ANTIGUA (con el problema de carrera)
server.listen(PORT, () => {
